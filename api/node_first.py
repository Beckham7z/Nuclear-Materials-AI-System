...
class LocalQueryEngine:
    ...
    def _get_final_response(self, results: List[Dict[str, Any]]) -> str:
        ...
        {chr(10).join(f'Document {i+1} (DOI: {r["doi"]}):/n{r["response"]}' for i, r in enumerate(results))}
    ...
    async def _get_final_response_async(self, results: List[Dict[str, Any]]) -> str:
        final_message = f"""Please carefully analyze the following document content and synthesize a comprehensive academic response.
        ...
        """
        try:
            summary = await self.chat_client.get_response("chat", user_message=final_message, kwargs={"temperature":0.7, "max_tokens": 8192,"stream":False})
            return summary
        except Exception as e:
            logger.error(f"LLM调用失败: {str(e)}")
            return ""

    async def _NodeFirstQuery(self) -> Dict[str, Any]:
        ...
        scores = await self._rerank_results(entity_id)
    ...
    async def _NodeFirstQuery_async(self) -> Dict[str, Any]:
        ...
        concurrency = getattr(self.global_config.rag, 'concurrency', 10)
        sem = asyncio.Semaphore(concurrency)
        async def summarize_node(node_info):
            node = node_info["node"]
            description = node.get("description", "")
            source_id = node.get("source_id", "")
            description_list = description.split("<SEP>")
            ...
            for src_id in source_id_list:
                ...
                if source_content:
                    for doi, content in source_content.items():
                        message = self._format_doc_chunks({doi: content})
                        await self.chat_client.get_response("chat", user_message=message, kwargs={"temperature":0.7, "max_tokens": 1024,"stream":False})
                        response = await self.chat_client.get_response("chat", user_message=message, kwargs={"temperature":0.7, "max_tokens": 1024,"stream":False})
                        return {
                            "doi": doi,
                            "source_id": src_id,
                            "content": content,
                    ...
                    logger.warning(f"未获取到source_id={src_id}对应的段落内容")
            return None
        tasks = [summarize_node(node_info) for node_info in top_k_sorted_info]
        collected_docs = [doc for doc in await asyncio.gather(*tasks) if doc]
        final_response = await self._get_final_response_async(collected_docs)
        return {"text": final_response}

    async def Page_Display(self):
        final_response = await self._NodeFirstQuery()
